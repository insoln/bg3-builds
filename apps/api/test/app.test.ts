...
describe("API", () => {
 ...
 it("maps provider phases to honest analysis details", async () => { const store=new InMemoryConversationStore(); const c=await store.create({}); const phasedProvider:MessageProvider={complete:vi.fn(async(_messages,sink)=>{sink.status("working");sink.status("tool");sink.status("paused");return [{role:"assistant" as const,content:[]}];})}; const app=buildApp({store,provider:phasedProvider}); const response=await app.inject({method:"POST",url:`/api/v1/conversations/${c.id}/messages`,payload:{content:"Help"}}); const working=response.body.indexOf('"detail":"Analyzing your request"'); const tool=response.body.indexOf('"detail":"Checking game data"'); const paused=response.body.indexOf('"detail":"Continuing analysis"'); expect(working).toBeGreaterThan(-1); expect(tool).toBeGreaterThan(working); expect(paused).toBeGreaterThan(tool); expect(response.body.indexOf('"status":"complete"')).toBeGreaterThan(paused); await app.close(); });
